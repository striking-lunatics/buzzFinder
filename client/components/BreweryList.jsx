   ...
   _createBreweryComponents() {
      ...
      if (this.props.brewerys) {
         return this.props.brewerys.filter((beer) => beer.streetAddress && beer.openToPublic == "Y" && beer.locationType != "office" && beer.brewery.images).map((beer, index) => {
            return <BreweryItem key={index} url={beer.brewery.website} name={beer.brewery.name} description={beer.brewery.description} address={beer.streetAddress} zipcode={beer.postalCode} distance={beer.distance} type={beer.locationType} icon={beer.brewery.images.icon} image={beer.brewery.images.squareMedium} socialAccounts={beer.brewery.socialAccounts}/>
         });
      } else {
